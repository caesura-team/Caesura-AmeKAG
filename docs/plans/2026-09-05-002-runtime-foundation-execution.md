...
路线图保留 U1–U29，底层优先，Studio 暂停。2026-09-08 用户明确恢复继续开发，取代此前“U11 后暂停”的安排。2026-09-24 接续状态见[当前待办](2026-09-05-003-runtime-foundation-todo.md)：U11–U21 已交付，U22/U23 的包与来源门禁已有实际证据，U26/U27 的独立增量继续验收和整合。完整路线图尚未完成，本记录不把排期当成完成证据。
...
此表保留早期执行阶段的来源和边界。U18 之后的当前进度以[接续清单](2026-09-05-003-runtime-foundation-todo.md)及各单元的新执行记录为准；“待继续执行”等旧状态不重新开启已交付工作。

| 单元 | 状态 | 现状与下一项证明 |
|---|---|---|
| U1 | 本机适用验收通过 | 53项证据测试、57项隔离mutation及真实run→collect→verify通过；无自动RC-GO；CI发布身份继续由U23承担 |
| U2 | Expo 三 lane 接入通过，后续配置继续 | 第六次在74c07ea5完成macOS完整profile、iOS Simulator1281项C++及未签名device编译；第七次在worker分配前因免费额度耗尽而NOT_RUN，不升级源码证明。PR17已合并；合并后Windows Release Web性能失败保留原结论，完整配置/sanitizer与性能基线仍待完成，详见2026-09-08-003 |
...
| U12 | 已交付 | 最终干净daec64bf完整Debug profile11/11、Cpp1292/1292、Lua147/43、CTest25通过+1预声明AI跳过、严格核验通过；完整Web496/496及原预算通过。PR #13合并master99a65d16，masterCI34206114925的10个任务全部成功 |
| U13 | 已交付 | AST差异及六组真实跨路径语料通过；干净43394fa0的Web498/498、Debug profile11/11、Cpp1292/1292、Lua147/44、CTest25通过+1预声明AI跳过及严格核验通过。PR #15合并master95bb9836，masterCI34209414785的10个任务全部成功 |
| U14 | 已交付 | 缓存兼容、全场景preflight、最终包及Node22 Unicode复制均有真实回归。PR16最终4927355b的候选CI通过，合并6f568b0a后10个作业全部成功；共同性能基线问题仍归U2/U27，详见2026-09-08-004 |
| U15 | 已交付 | 31c22afa完整Debug profile与严格核验通过，Cpp1325/1325、Lua147/46、CTest25通过+1预声明AI跳过，完整Web511/511。PR18最终afd9e703候选CI通过，合并14b43ade后masterCI34266968673的10项任务全部成功；此前失败保留，详见2026-09-09-007 |
| U16 | 已交付 | 固定D3D11/GL矩阵18/18、82张截图；完整Debug/C++1333/Lua/CTest和严格证据核验通过，PR #19候选CI7项成功后合并074f5f7c；详见2026-09-09-007，未测设备/复杂效果范围保留 |
| U17 | 已交付 | SDL事件、触摸/IME、交错暂停、真实SoLoud混音与runner回归通过；完整Debug/C++1361/Lua147+48/CTest及严格核验通过，最终CI7项成功后PR #20合并0c7e2e86。设备未测及本地Web固定预算失败保留；CI完整Web511/511通过，详见2026-09-12-008 |
| U18 | 执行中 | 实际排队超时后仍执行、帧日志刷屏、默认错误后同批命令仍执行和真实Lua分配失败panic均有红灯；错误停止/限流定向绿灯，队列确定性测试与退出背压继续，详见2026-09-12-009 |
| U19–U26 | 待继续执行 | 按唯一计划逐项实现和验收；已有Apple准备不代替真实设备/SDK/最终包证明 |
| U27 | 共同Web成本修复通过本机完整检查，基线/长跑待执行 | 固定C/P保留吞吐FAIL与捕获计数NOT_MEASURED；减少临时分配、合并等价已读比较及nil暂停短路后，a699bc5c完整Web511/511、原12性能预算及严格核验通过。原失败记录未替换；至少10样本/3独立进程Release基线与一小时长跑仍未完成，详见2026-09-09-006 |
| U28–U29 | 待继续执行 | 当前能力/平台声明随证据同步，完整候选与最终包验收尚未完成 |
