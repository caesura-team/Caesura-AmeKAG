...
### 2026-09-24 — 维护运行器、轨迹和夹具修正后的完整门禁

干净提交 `3a76727e4e94004c50766bf8ea42db15e54d05cf` 完整Windows Debug运行 `7e7345f2-3eda-4fea-b21e-b0c2528f6696` 通过。全量构建、C++1477/1477及444441断言（0失败/0跳过）、Lua147/147+56/56通过，CTest62项中61通过、0失败、1项预声明可选AI跳过，用时643.47秒；原Web协议33项及三个Soak合同均通过。execute/collector/strict实际退出0，全部owned清理完成且无超时/强杀，源码fingerprint `d262f4c7312acb1aed9c897d7452b31164a42582cf59dead8a571bb3817f5083` 首尾稳定，输入未变。

原run摘要 `8188da6abfeb9cc7945aa69c1551285c9ab7e95f6c222f9e804210186ddf865d`，根重算29份流/报告及CTest XML的审查摘要 `7a764a467a40815d3e270363804bb6609d84aaa30f42ecf8b933cd12828b89c6`，目录 `artifacts/validation/u27-owned-trace-full-02`。full-01仍是原失败，不改标、不覆盖。本次只接受该提交的完整Debug门禁；同进程上下文重建、冷恢复、故障负控、120秒短版和连续一小时长版继续实施，U27整体未完成。

### 2026-09-24 — 同一进程中的 Engine 销毁与重建诊断

在保留原18项轨迹回归的基础上增加13项上下文序列回归，空检查器得到31方法中35个失败子断言；实现后同一31方法全部通过。单个上下文仍调用原轨迹检查，每轮20个完整预热周期、独立资源基准与完整关停必须成立；上下文代次不可重复，PID/创建时间不变，重建间隔不能贡献无进展的长时间。Lua/private增长预算继续绑定第一轮预热最大值，后续预热不能重置它。短/长合同要求每100个测量周期重建，诊断单独允许每2个测量周期；当前维护CLI只开放两种diagnostic，不发布short/long通过选项。

旧实际Release探针对新增上下文参数退出1并明确Usage，未初始化；保留其二进制、源码/输入锁及原流，摘要 `4319b343f7634c0271f76622b6e67d7f75a308d88519098dff0bc39d6cf8061e`。新探针在Engine.run返回后调用shutdown，再在独立词法作用域末销毁Engine；确认BackendRegistry四个后端引用清空后才创建下一轮。每轮使用独立目录、原始result/events与完整PNG，外层独立进度流逐项对应本地轨迹及销毁事件。维护运行器核对所有目录、结果、身份、时钟、模块及图像，仍 `accepted_soak=false`。

真实context-diagnostic-01在源码fingerprint `a404e256c071892c2c7cc7f7ee719bff661f5fe156535f4be5ad1875c30259a6` 首尾稳定时通过。实际PID25068/creation134346665256705514完成两个上下文，代次1→2，各20预热+2测量；四个测量周期，连续测量区间15.9516876秒（包含第二轮预热），整个探针29.5943793秒、外部owner观察29.797秒。132张真实PNG均由维护解码器检查完整像素A等于restored且不同于B；根重新核对文件摘要与两轮轨迹，没有把两次独立进程时长相加。

第一轮预热private最大值374607872字节，之后包括第二轮预热的最大值364191744字节；Lua对应1295885和1304505字节，增加8620字节。只比较已声明的基准/预算，不用全部样本的最小值冒充初始基线，也不据此声称一小时无增长。原run摘要 `db1b2cef921d4279f074516336d4a15998c78e06f279da7960b16ac35b986f01`，contexts摘要 `c2aa42defd202f896a52f1497b1a208e1dce2acf0296330a12b7a5abc2e10734`，真实模块记录摘要 `e7b9373886ecb4a74b7cd9179ae62a0f2dd2fe6ee6a33d0ff7e3245c348640f5`，根审查摘要 `27d75f61c56ff43f37657fb6d3165e02af1773514a5f8c3df058d535044d2835`。首次辅助审查误把已退出PID的process_identity接口当作返回None，实际抛出Windows87；改用针对同一PID的CIM只读查询确认零进程，未重跑实际诊断。

实际三个Soak CTest入口3/3通过、10.41秒，收据摘要 `8470fcc635ce61e133ac476acdb2cfdb6429ca2c7f192fbf8254ecaf08f04977`。新增代码经根审查，未发现剩余可行动问题；没有修改生产后端或公共API，也没有减少现有测试。完整Debug证据仍绑定前述3a76727e；本增量取得Release探针构建、实际上下文诊断和定向维护回归，尚未完成新的完整集成门禁。冷恢复、实际资源/worker/损坏存档/停滞负控、120秒短版和一小时长版继续执行，物理可听输出及OS设备丢失仍未测。

### 2026-09-24 — 三个独立原生进程的冷恢复与损坏存档对照

新增冷角色轨迹检查、原生producer/consumer/corrupt模式和维护入口 `scripts/run_engine_soak_cold.py`。维护入口要求调用者固定普通诊断的原始摘要，确认源码、Release二进制/DLL及foundation配置完全匹配后，才准备三个新的隔离运行目录。每个原生进程在初始化后等待外部owner确认；控制器独立枚举真实可执行映像、邻接SDL及系统D3D11，再原子发布PID/创建时间确认。消费者读取生产者原样保留的CAES文件，禁止重新保存；损坏对照只翻转同一存档副本最后一字节的一位。所有冷角色的测量周期和测量时长均为零，不计入连续soak。

12个新增轨迹回归先在空实现上实际RED，43方法产生47个失败子断言；实现后同一43方法全部通过，原31个方法AST不变。旧Release探针对冷角色参数退出1并打印Usage，未初始化，原结果摘要 `922a58135bd4e6050ffb311e7d97fef8fdd81227a9d2a9d4cd55d3712d998ef4`；这是未实现入口的RED。当前源码重新构建Release后，普通22周期诊断通过，20预热后2测量周期/1.3200938秒，外部owner观察14.969秒，66张PNG完整像素满足A=restored且不等于B。基线原run摘要 `d744c6acf79fbd85b3fe9893efebab58b1d81e3b9e4c741e7bcab3765b00788a`。

真实冷角色PID/创建时间分别为26628/134346678818035676、23108/134346678831345211、21944/134346678850432423。生产者保存并保留1982字节CAES，摘要 `d6ff1ba8fc8ffa5ae7d2657b2c462f3b539b6cba1bc7d60b63b10ab3328dedef`；正常消费者由未保存的777/B/consumer-secret上下文恢复为1/A/producer-secret，实际ctx对象替换，恢复后整帧RGBA与生产者完全相同。损坏消费者的实际load被拒绝，ctx对象、777/B/consumer-secret及前后整帧像素保持不变。Lua报告 `Save slot has no representable state table`；原生Storage日志另有 `Decryption failed (wrong key or corrupted data)`，两个层次的错误分别保留，不混写为同一返回值。

冷运行原run摘要 `4c194ce463bf9fde688db13c32c94a0fc86ecea83979c91a1ca8194f52f2fc1c`。基线与冷运行源码fingerprint均为 `8968926ed153fe0ec42a739452ef943543176dfbe1dcf0c273aff1c658a8bb16`，首尾稳定；三个实际进程exit0、owned cleanup COMPLETE、无超时强杀，后续CIM确认已退出。根重新核对896份原始引用，独立解码基线66张及冷角色5张PNG、验证存档逐字节关系并重跑轨迹谓词；审查摘要 `ac5040b53e4b66281776ed745f62dbed52916ee9032a1858144d155a740ba37c`，位于 `artifacts/validation/u27-soak-workload/cold-controls-01/root-review-01.json`。首次辅助审查按Windows默认GBK读取含中文路径日志失败，保留原脚本；改为显式UTF-8后完成，没有重跑或替换原始诊断。

实际CLI错误摘要对照仅改一个字符，退出1并在创建任何角色或启动原生进程前拒绝，输出目录只有失败run.json；收据摘要 `8252599c8d15f103403204e6e37e3f11e7f7ed47ce30abc0fec8a32d3cba6346`。三个Soak CTest入口3/3通过、10.39秒（轨迹43方法、运行器11方法、静止点14方法），收据摘要 `d1b3cbf9244cf975db4ba893ac460335a9bdb3cbbb0b29d6fcd05223219cbad5`。根已审查本增量，无剩余可行动发现；未改变生产后端、公共API或原数字预算。此证据关闭当前冷恢复及损坏存档对照切片；实际保留资源/worker/停滞负控、120秒短版、连续一小时长版、多进程性能基线和新完整门禁仍待完成，U27及全计划继续未完成。

### 2026-09-24 — 实际保留资源、已进入 worker 与初始化后停滞负控

三个新故障角色在旧二进制上均实际退出1并报告Unknown cold role，未初始化；原RED摘要 `7aee9815cb2a2743af9937bba2964384db4511a589a9a85c5044a50c40dcb4ce`。随后仅扩展测试探针及维护控制器 `scripts/run_engine_soak_faults.py`，没有改生产后端、原静止点谓词、事件语法或任何数字预算。资源在第20预热边界后以独立颜色创建并保留；worker在原异步/音频/回滚结束后、静止点前提交，实际promise屏障确认进入线程，再等待跨越完整静止点预算的release future。析构及显式shutdown前释放屏障，防止负控自身阻塞清理。停滞在外部确认OS owner和真实模块后进入Engine owner回调，保留最后初始化/停滞检查点，由外部一秒进度期限终止。

同源普通Release构建及22周期诊断先通过，原run摘要 `84cd8c0716d8c8cf6a8d3a58e8ecf9c745b8744226d9f62155911c21e37fe432`；故障原run摘要 `a0378a09845cf70027dbe97d29ede11052f66a20c5cfd0b06a9dda8f5606dd50`。源码fingerprint `249d1a7afae926a0f3da0870622a81537c24d2845b033c89db72850dc769d6d4` 首尾稳定。实际结果如下：

- 资源进程PID3180/creation134346689409429874完成22周期并正常退出0，但原轨迹谓词明确拒绝第21/22静止点：textures由6变8、frameBuffers由0变1、逻辑textureBytes由18432变18436。实际保留texture63/target81直到测量后才请求销毁；没有因退出0而通过soak。
- worker进程PID16376/creation134346689795270862的真实job43进入后保持workerPending=1；第428帧进入静止阶段、第549帧越过原120帧上限，实际失败原因是固定静止点期限。原check_quiet只报这一项未完成债务，没有把其他失败混作worker证明。release后实际退出1、正常owned清理，无超时/强杀；失败trace没有改写成PROBE_COMPLETED。
- 停滞进程PID21952/creation134346690177604309由就绪后的进度期限拒绝，原观察错误为Progress deadline expired after readiness。owned收据为STOPPED、actual exit1、stop_requested=true、cleanup COMPLETE，forced_kill=false、timed_out=false；这里是监督器请求的受控停止，不把它改称底层总时限超时或正常完成。原fault_stall事件和stop请求均保留。

共用运行/清理路径修改后，同源码当前二进制的三个冷角色再次通过，原run摘要 `50b9dd5ba113f2f6ceab54cf919c243c1b7e357fb2f79b510402dfdfdf36f60e`。三个Soak CTest入口3/3通过、12.27秒，收据 `dfc4f61b8de61f5a21e91c4b7cff303d0a4098150ce1108d5d45f0f58ac92d0c`。根核对2200份原始引用及所有200张PNG摘要，按完全相同的编码字节复用解码结果（实际只有2份不同图像），仍逐文件绑定整帧RGBA关系；对原始证据的8项内存副本改动均被见证检查拒绝，属于检查器反例而非新增物理运行。三个故障身份各不相同，CIM确认所有观察到的进程均已退出。根审查摘要 `8353d1be04cd70d1f7b0c9a6137e883c317b6e16cb33e2a0867d2fc93fa4381e`，无剩余可行动发现。

本批关闭计划中的三个实际故障对照切片，不接受短版、长版或完整U27。完整Debug证据仍绑定3a76727e，新的完整门禁、120秒短版、连续一小时候选长跑以及至少10样本/3个独立进程的Release性能比较继续执行；物理可听输出和OS设备丢失不在上述结果内。

### 2026-09-24 — 短版/长版维护入口与前置证据绑定

维护CLI增加short/long模式，保留diagnostic和context-diagnostic原入口。short必须显式选择同源码、二进制、配置的冷恢复及三个实际故障对照原始摘要；long还必须选择同一组对照支撑的已通过short报告。控制报告引用的普通诊断也按原摘要回读，运行前后再次确认全部绑定，不能用报告自身声称的摘要替换调用者固定值。合成前置报告的7个新回归在空实现上产生9个失败子断言，实现后与原11方法一起18/18通过，测试方法AST不变；这部分合成报告不计为实际原生验证。

短版原生参数固定为每上下文20预热+100测量、至少2上下文和120秒连续测量；长版保持同样重建节奏，至少6上下文、3600秒及原谓词600测量周期最低值。实际总期限分别为600/4200秒，启动30秒、进度10秒，原逐周期和静止点预算不变。`accepted_soak`只在long全部原始轨迹、图像、前置证据和源码复核通过时为true；short单独记为SHORT_REVIEWED，不代表完整U27或发布验收。
