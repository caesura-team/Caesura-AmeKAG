...
独立后台执行的完整profile `raw/windows-debug-u12-03` 正常完成并保留FAIL，run ID `d0d25dd9-1a1b-41ed-9f17-f0d1ae24f77f`。Debug构建、Cpp1292/1292与399001断言、Lua147/43、验证工具及耦合/注册均通过；CTest仅Golden VN失败，外部AI smoke按预声明条件跳过。

Golden旧门禁以至少50次点击代替分支运行证据；全文揭示修复后真实分支47次点击即可完成。门禁现在要求正常退出、DONE、原terminal token边界、实际route变量和完整揭示过的分支台词；错误route/text均通过真实driver负控制验证。Python门禁正反回归先9通过/3失败，修复后12/12；最终LF写入补强随后纳入整套检查。`golden-u12-targeted-02` 完整Golden通过，审查无阻断项。
