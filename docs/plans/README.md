...
2026-09-24 恢复后当前接续：完整目标仍为全部U1–U29，底层优先、Studio暂停。[U29隔离整合](2026-09-24-017-foundation-integration-execution.md#2026-09-24-恢复后完整-debug-与托管失败修复续记)的当前源码为 `53721aa9f7fe0a917575b41882ae9e8b2949280b`（`codex/u29-hosted-fixes`）。三文件修复已提交，两项独立静态审查无可行动问题；新提交尚无完整门禁或CI通过结论。

恢复后的前一候选 `ed6e9c267d03241aa5c01b203dd0323b44c31ca8` 已完成Windows基础Debug完整11项及根/独审：C++1524零失败零跳过、Lua147/56、CTest71通过及1项预先允许的AI跳过。ed6托管run35969560756/attempt1终态9成功、3失败；Linux独立C++语音用例、macOS工具实际映像身份及聚合gate原错误的完整失败归档已恢复核对，原失败不改写。

53721aa9的音频补丁仅建立真实ManualMix测试前提，新C++未编译/运行；Mac协调器Windows目标入口23/23通过，4项POSIX专用用例及真实Apple执行未测。ed6 FFmpeg Debug02原始终态PASS，根终态复核和独审均通过；ed6 SDK inputs02准备及独审通过，Live2D完整Debug已启动、无终态，首轮Live2D输入遗漏导致的编译REJECTED保留。两项运行证据不迁移到537候选。旧f87最终包字节及冷恢复输入已核验准备，冷恢复尚未执行。
...
具体原始引用、摘要和未测边界见[U29恢复后记录](2026-09-24-017-foundation-integration-execution.md)；[当前待办](2026-09-05-003-runtime-foundation-todo.md)保留全部U1–U29。U23服务端新只读确认不替代丢失的原mutation收据；U24设备、U25配额、U26真实SDK/账号、U27未决性能、U28声明和U29整合/最终包/回退继续按原合同验收。平台生成锚更新到537代码提交，历史能力仍为 `NOT_REVERIFIED`，不升级真实平台或发布声明。
