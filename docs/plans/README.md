...
2026-09-24 恢复后当前接续：完整目标仍为全部U1–U29，底层优先、Studio暂停。[U29隔离整合](2026-09-24-017-foundation-integration-execution.md#2026-09-24-托管终态七文件修复与误删后的接续)的最新代码候选是 `09601b20eeeba6053baec4758ee51a1ef0b222e9`，七文件修复已提交，删除前交叉审查无可行动发现；源码与记录已恢复到E盘，四份关键树的文件、索引和符号链接已独立核对。记录树 `40fa2421` 的五份文档现已同步。

原71c托管run35961032218/attempt1终态为6成功、4失败、2跳过，macOS Debug、Windows Debug、Linux Release和聚合失败。四份失败小ZIP及完整macOS诊断ZIP、原run/jobs和四份失败job日志已恢复并核对整包摘要；Linux Release另取回三个原诊断成员，仅验证成员CRC和SHA，未验证整份诊断ZIP。原失败保持，未重跑CI。

删除前eda本机完整Debug已独审，完整Release已根复核；Release独审未完成。两套完整根证据目前缺失，旧计数和摘要仅保留为历史观察。09601的Python78/18/33/15及相关红负控也是删除前观察，完整根报告未找回；两个C++改动尚未编译或运行，新候选完整门禁待执行。幸存的CLI、Linux与Temp原件保留各自范围，不拼接补造完整门禁。

恢复清单、当前可核原件与缺口见[U29恢复后记录](2026-09-24-017-foundation-integration-execution.md#已恢复原件与当前缺口)；[当前待办](2026-09-05-003-runtime-foundation-todo.md)保留U1–U29全范围。U23服务端最小变更的新只读确认仍匹配，无需重复mutation；U24设备、U25配额、U26真实SDK与账号、U27未决性能、U28声明及U29整合/最终包/回退继续按原合同验收。生成锚点更新不升级任何历史平台能力结论。
