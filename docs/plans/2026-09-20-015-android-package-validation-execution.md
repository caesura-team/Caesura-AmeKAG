...
## 签名 AAB 的独立 base manifest 实际读取

在干净959f1bc95cffa63750b84d0b11936564596b46f0上，以实际bundletool1.17.1 dump manifest读取第五次执行保留的同字节签名AAB（fb83b7726c39a5e684a2a46bbfb19824a1aa19f5f1114f3b7c32fa54bb9e8e71）。解析原XML确认base package=com.caesura.app、versionCode1、versionName1.0.1、minSdk24、targetSdk35，与原外锁expected相同。原XML摘要48cbc91528796aea687a30a328b3a96788bf7676dd01a09aebd00df6129ce649，stderr为空；实际Java PID18404/creation134343341527180227退出0、无超时强杀、cleanup COMPLETE。
...
## v2 完整执行与桌面复制输入缺口（2026-09-20）

干净9e78fdb9的第六次完整请求在输入阶段拒绝未声明`scripts/__pycache__/web_package_runtime.cpython-312.pyc`，尚未执行编译；原失败收据保持。实际隔离36个普通未跟踪缓存文件、1205987字节到本工作树ignored证据目录，逐项核对源/目标范围和哈希，未改 tracked 文件或放宽输入扫描。新第七次请求使用相同clean源码重新执行，driver实际exit0、源码前后fingerprint7e34eb81234eac693a41a8043f69c0529cbd16bdb2383feeed5f65c483518bf3相同，私密目录清理COMPLETE；原android-validation.json摘要0d878ac28d60db1b3790ecc0b8747c0662395037d3e0f20936da1debac04c8a4。

后续独立只读审计发现真实构建执行了桌面POST_BUILD复制：web/dist的405份ignored文件和projects的38份ignored文件，共44792680字节被复制到native输出，未在source.files内。原demo的45份均tracked，不能混入443未知文件计数；但原递归扫描未覆盖demo这一边界也被记录。审计逐最终ZIP成员核对了APK/AAB，stage与签名前后业务集合相同，未发现这443份副本变成额外最终包条目。原6490源路径212826743字节已按执行收据重哈希、两个Git链接终点相同；审计后新增测试明确单列为当前dirty，不冒称当前工作区仍是旧执行时clean状态。该输入缺口使原driver成功不能证明整个构建输入封闭，07原报告不改写。
