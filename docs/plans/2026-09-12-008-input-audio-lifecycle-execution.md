...
- 输入定向修复验证，以及触摸抬起/取消、完整设备/手指身份、长按互斥、焦点变化后的待消费点击。
- IME真实命令提交/取消、原生SDL分发及适用设备路径；暂停期间游戏逻辑。
- 适用设备运行。2026-09-12执行 `adb devices -l` 启动本机ADB服务后列表为空；没有Android连接/触屏/设备运行通过证据。
- 独立增量审查、完整Debug/C++/Lua/CTest、Web受影响范围、发现门槛和文档生成同步、候选CI与交付。
...
- 完整Debug/C++/CTest候选门禁、CI及交付尚未执行。本节所列定向通过不替代这些门禁。
